...
\newcommand*{\Source}{
    \input{main_files/01-intro}
    \input{main_files/02-literature}
    \input{main_files/03-methodic}
    \input{main_files/04-metrics}
    \input{main_files/05-results}
    \input{main_files/06-conclusion}
}
...
\newcommand*{\TitleEN}{%
    Automating the process of code metamorphism using large language models%
}
...
\newcommand*{\AuthorsInfo}{%
    \thanks{Давид Каренович Мурадян, МГУ им. М.В.~Ломоносова, (email: username@domain.ru).}%
    \thanks{Олег Вячеславович Цветков, РТУ МИРЭА, (email: username@domain.ru).}%
}
